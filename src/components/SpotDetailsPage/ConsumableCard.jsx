...
function ConsumableCard(props){
    ...
    return(
        ...
        >
          ...
          <Typography><h2>What I ate @ {spot.name}</h2></Typography>
          </AccordionSummary>
          <AccordionDetails>
...
{spot.consumables && spot.consumables.map((singleConsumable) =>{
  return                  <div key={singleConsumable._id}>
  ...
  <Card sx={{ maxWidth: "100vw" }}>
    ...
    <div>
      <Typography variant="h5">{singleConsumable.name}</Typography>
      <Typography variant="body2">{singleConsumable.tagline}</Typography>
    </div>
